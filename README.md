...
공개 API가 없는 커머스 사이트(무신사·29CM·W컨셉·EQL·SSF·자사몰)에서 자연어 요청 하나로
상품·랭킹을 모아 **검증된 가설을 PDF 리포트**로 내는 **Claude 스킬**과, 그 스킬을 만드는
과정의 스펙·테스트다. (HTML 산출은 D27로 폐기됐다 — 2026-08-03 생성기까지 삭제.)

**팀원에게 배포되는 것은 `skills/` 하나이고, 사용 안내는 `skills/README.md`다.**
이 문서는 저장소를 여는 개발자용이다.
