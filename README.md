...
**페이지 안에서 JS를 실행**할 수 있으면 된다(Playwright MCP 등 — 특정 도구에 묶여 있지 않다).

무신사 랭킹, 29CM 카테고리·브랜드 목록은 **서버 HTML에는 상품이 없고 브라우저로 열어야
렌더된다.** 그래서 이 스킬은 화면을 읽는 것을 기본 경로로 삼는다.

브라우저 도구 없이도 사이트가 화면을 그릴 때 쓰는 JSON으로 낮춰서 수집할 수는 있지만,
**전량 보장이 깨진다.** 실측 예: 29CM 브랜드 카탈로그 435개 중 JSON 경로는 422개만 줬고
(누락 13건 중 11건이 품절), **어떤 상품이 빠질지 예측할 방법이 없었다.** 화면에 찍힌
총계와 대조하는 것만이 전량의 근거다.
...
수집은 에이전트가 브라우저로, **검증·집계·리포트 생성은 코드가** 한다. 둘 사이는
