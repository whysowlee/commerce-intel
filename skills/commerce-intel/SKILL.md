...
**프록시 준비가 끝나면 `insight-analyst` 서브에이전트를 스폰한다.**

수집·적재에서 쌓인 컨텍스트가 분석 판단과 인사이트 작성 품질을 떨어뜨리기 때문이다.
서브에이전트는 도메인 규칙(변수 역할·인과적 함정·어조·정직성)이 시스템 프롬프트에
포함된 채로, 수집 컨텍스트 없이 깨끗한 상태에서 시작한다.
정의: `.claude/agents/insight-analyst.md`.

**스폰 시 넘기는 정보:**
- `--db <DB 경로>` `--context "<context>"` `--target "<대상 이름>"`
- EDA에서 발견한 데이터 특이사항 (결측 패턴·시계열 갭·생존편향 경고 등)
- 사용자의 추가 지시 (있으면 원문 그대로)

서브에이전트가 수행하는 것:
- 분석 계획 수립 → 1차 실행 → 액션 플랜 작성(D64) → 2차 실행 → 수치 재검증(D52)
...
| 파일 | 무엇 | 누가 |
|---|---|---|
| `insight-<대상>-<ts>.pdf` | 강한 주장 + 약한 단서. 짧다 | 전원 |
| `detail-<대상>-<ts>.pdf` | 판정 근거·EDA 프로파일 | 근거를 볼 사람 |
