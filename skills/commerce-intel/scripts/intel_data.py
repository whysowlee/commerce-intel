...
#
# **예외 — purchase_count (D76, 2026-08-07 사용자 결정):** 세 플랫폼 모두 수치형
# 판매량이 없어(100% 결측) 누적판매는 구간 하한을 purchase_count 정수 칸에
# **그대로 적재한다**. 위 함정(하한≠실값, 평균/비율 오염)은 사용자가 인지하고
# 수용했다. 원문은 purchase_count_display에 보존되므로 하한임을 되짚을 수 있다.
# view/like는 이 예외 밖이다 — 여전히 구간 축(band)으로만 쓴다.
