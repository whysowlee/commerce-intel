...
description: EQL(eqlstore.com, 한섬 편집숍)에서 상품 데이터를 수집해 commerce-intel 데이터
  ...
  "eqlstore 신상품 훑어줘". **사용자 지시 건별 수집만 한다(D30)** — 크론 무인 축적·랭킹
  주기 축적에는 쓰지 않는다(그건 무신사·29CM 몫이다). 다른 플랫폼 요청이나 상품 구매
  대행에도 쓰지 않는다.
...
## ⚠️ 수집 신분 — 건별 수집만 (D30, 2026-08-03 사용자 확정)
...
- 그래서 수집 방식은 **ⓐ 사용자 지시 건별 수집만**이다 — **크론 무인 축적 금지.**
  랭킹 축적은 무신사·29CM에서 한다. 이 결정은 SPEC-INTEL D30에 기록돼 있다.
- UA 기반 기술 차단은 없다(전 요청 200, 2026-08-03 실측). 차단이 없다는 것이
  축적 허가가 아니다 — 선은 robots 정책과 D30이 긋는다.
