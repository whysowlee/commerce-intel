...
# SPEC v15 — 축약 표기(`1.2천`·`판매 9만개`)는 정수로 파싱해 담는다. 반올림된 노출값이라는
# 사실은 표 각주로 밝힌다. 파싱을 허용한 것이지 정밀도가 생긴 것이 아니다.
ROUNDED_NOTE = (
    "조회수·누적판매·좋아요처럼 사이트가 <strong>축약해 보여주는 값</strong>"
    "(<code>1.2천</code>·<code>판매 9만개</code>)은 정수로 옮겨 담는다 — "
    "반올림된 노출값이므로 자릿수 규모의 비교에만 쓴다. "
    "반면 <code>300회 이상</code> 같은 <strong>구간 표기는 정수로 바꾸지 않고</strong> 문구 그대로 싣는다."
)
    ...
    if multi:
        ...
        out.append(
            section(
                ...
                "(값이 없는 행은 방향과 무관하게 뒤로 간다). " + ROUNDED_NOTE,
            )
        )
    return "".join(out)
